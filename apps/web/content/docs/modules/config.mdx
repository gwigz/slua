...
The `config` module reads settings notecards and applies parsed values
...
```ts
import { loadConfig, onConfigChanged } from "./modules/config"
...
<auto-type-table name="ConfigCallback" type="
export interface ConfigCallback {
  /** Whether the notecard was read successfully. `false` if the read timed out. */
  ok: boolean
  /** Error message when `ok` is `false` (e.g. `'timeout'`). */
  error?: string
}
" />
