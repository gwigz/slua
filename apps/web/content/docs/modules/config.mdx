...
The config object you pass in decides which keys the parser accepts and how
it coerces their values. The type of each default value picks the behaviour:
...
The parser ignores notecard keys that are missing from your defaults, so
comments and unrelated keys cause no errors. The lljson parser assigns matching
keys directly, with no type coercion.
