...
ready, then return the value directly.
...
```ts
spawn(() => {
  const choice = dialog(-99001, ll.GetOwner(), "Pick one:", ["Yes", "No"])
...
```ts
spawn(() => {
  const res = httpRequest("https://example.com/api", [HTTP_METHOD, "GET"], "")
...
```ts
spawn(() => {
  const detected = sensor("", NULL_KEY, AGENT, 20.0, PI)
