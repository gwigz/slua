...
The `yield` module uses Lua coroutines to flatten SLua's callback-based APIs
into sequential code. Call `spawn()` to start a coroutine, then use the wrapper
functions inside it. Each yields the coroutine until the result is ready, then
returns `[ok, value]`, where `ok` is `false` if the request timed out.

```ts
import { spawn, requestAgentData, sleep } from "./modules/yield"
