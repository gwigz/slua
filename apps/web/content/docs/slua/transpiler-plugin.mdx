...
| `httpRequest(url, body)`       | `ll.HTTPRequest(url, {...}, body)`           |
| `castRay(start, end)`          | `ll.CastRay(start, end, {...})`              |
...
Functions like `ll.GetObjectDetails`, `ll.GetParcelDetails`, `ll.GetPrimMediaParams`, `ll.GetLinkMedia`, and `ll.ParcelMediaQuery` accept an array of flag constants and return a corresponding list of values. The type definitions map each flag to its return type, so destructuring gives you fully typed results:
