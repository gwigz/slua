...
Rewrites `not (x ~= nil)` to `x == nil`. TSTL emits the negated form when double-negated null checks survive the AST (e.g. `!(x !== null)` in source), and the simplified form drops a `not`, a pair of parens, and matches hand-written Lua idiom.
