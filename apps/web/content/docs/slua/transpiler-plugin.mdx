...
The `@gwigz/slua-tstl-plugin` replaces TSTL lualib helpers with native Luau stdlib and `ll.*` calls, so less runtime code ships with your script. It also handles index adjustment, casing fixes, and optional output optimizations.
...
The plugin translates string methods to LSL `ll.*` functions or Luau `string.*` calls:
...
| `str.startsWith(x)`    | `string.sub(str, 1, #x) == x`                    |
| `str.endsWith(x)`      | `string.sub(str, -#x) == x`                      |
...
> **Plain-text literals.** For `indexOf`, `includes`, and the `startsWith` fallback, the `1, true` plain-text arguments to `string.find` are only emitted when the needle could contain Luau pattern characters.

> **`startsWith` / `endsWith` literals.** With a string-literal needle, both compile to a `string.sub` comparison. Empty literal folds to `true`. A non-literal `startsWith(x)` falls back to `string.find(str, x, 1, true) == 1`, and a non-literal `endsWith(x)` falls back to TSTL's default `endsWith` helper.
...
The plugin translates TypeScript bitwise operators to `bit32` library calls, since SLua has no native Lua bitwise operators.
...
Compound assignments (`&=`, `|=`, `^=`, `<<=`, `>>=`, `>>>=`) desugar to the same `bit32` calls.

#### btest optimization

Comparisons of a bitwise AND against zero fold into `bit32.btest`:
...
SLua's `ll.*` functions use 1-based indexing (Lua convention), but TypeScript uses 0-based. The plugin adjusts index arguments and return values from the `@indexArg` and `@indexReturn` JSDoc tags in the type definitions:
...
`Math.floor(a / b)` becomes the native Luau floor division operator `//`:

| TypeScript          | Lua output |
| ------------------- | ---------- |
| `Math.floor(a / b)` | `a // b`   |

This only applies when the argument is directly a `/` expression. `Math.floor(x)` with anything else stays as it is.
...
The plugin collapses a zero-parameter arrow function that only calls another zero-parameter function into a direct function reference:
...
Many LSL functions accept flat parameter lists of `[CONSTANT, ...args, CONSTANT, ...args]` pairs. The plugin ships builder functions that compile to those flat lists at build time.
...
Shape variants get their own methods, `.typeBox()`, `.typeSphere()`, and so on:
...
Each builder method name comes from the LSL constant, minus the set prefix and camelCased. `PRIM_COLOR` becomes `.color()`, `PSYS_SRC_BURST_RATE` becomes `.srcBurstRate()`, and `CAMERA_FOCUS_LOCKED` becomes `.focusLocked()`.

> Builder chains are entirely compile-time. The plugin walks the method chain AST and emits the flat Lua table directly, so no objects, arrays, or closures reach the runtime.

#### Options object overload

`$castRay` and `$httpRequest` also take an options object, which compiles to the same flat parameter list but works as an expression:
...
The return type is `MapObjectDetails<T> | []`, either the mapped tuple or an empty table when the target object doesn't exist. A truthiness check handles both cases:
...
When the flags array isn't a const tuple (e.g. a dynamic `ObjectDetailFlag[]`), the return falls back to a union array of every possible return type.
...
Inlines `arr.filter(cb)` as an `ipairs` loop instead of pulling in `__TS__ArrayFilter`. Disabled for files with more than one `.filter()` call, where the shared helper makes for a smaller script.
...
Bare `indexOf` calls without a comparison still emit `(find or 0) - 1` to retain 0-index style responses. As with the other `string.find` paths, the `1, true` arguments are dropped when `x` is a magic-free string literal.
...
Rewrites `not (x ~= nil)` to `x == nil`. TSTL emits the negated form when double-negated null checks survive the AST (e.g. `!(x !== null)` in source). The rewrite drops a `not` and a pair of parens, and it reads like hand-written Lua.
...
Only applies to `nil` comparisons. `not (x ~= 1)` and other non-nil inequalities are left alone.

## Keeping output small

A few TypeScript patterns pull in large TSTL runtime helpers. These are the ones worth avoiding.
...
<Callout type="info" title="Enforce with oxlint">
  ...
  It also flags the [truthiness gotcha](/docs/slua/gotchas#truthiness).
</Callout>
...
Type your records to allow `undefined` and assign `undefined` instead, which compiles to `nil`:
