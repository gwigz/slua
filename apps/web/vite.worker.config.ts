...
/**
 * After the worker is bundled, hash its content and emit a tiny TS module the
 * Next.js app can import to cache-bust the worker URL on every build.
 */
function emitWorkerVersion(): Plugin {
  return {
    name: "emit-worker-version",
    apply: "build",
    closeBundle() {
      const workerPath = resolve("public/playground-worker.js")
      const hash = crypto
        .createHash("sha256")
        .update(fs.readFileSync(workerPath))
        .digest("hex")
        .slice(0, 12)
      const out = resolve("src/playground/generated/worker-version.ts")
      fs.writeFileSync(
        out,
        `// Generated by vite.worker.config.ts -- do not edit by hand.\nexport const WORKER_VERSION = ${JSON.stringify(hash)}\n`,
      )
    },
  }
}
...
export default defineConfig({
  plugins: [injectNodeGlobals(), stubTstlResolve(), tstlLualib(), emitWorkerVersion()],
  // keepNames preserves Function.name / class names through minification so
  // runtime errors like "TypeError: a.ok is not a function" become diagnosable.
  esbuild: {
    keepNames: true,
  },
  build: {
