 ...
 *
 * The transform is generic, which means it also rewrites keys inside payloads
 * we do not own: the `params` of an `editor.*` command, and whatever a command
 * handler returns. Anything travelling through those needs keys that survive a
 * round trip. Values are never touched, only keys.
 */
